...
## 在线表格原生公式、粘贴与数字格式增强（2026-07-30）

状态：开发和验证完成，本节随本轮中文提交交付 `gitee/main`。

已完成：

- 石墨等外部表格复制内容支持从 HTML 单元格属性、嵌套公式节点、结构化剪贴板 MIME 和纯文本公式
  中提取公式；来源携带复制范围时按粘贴位置迁移相对、绝对和混合引用。来源只提供显示值时保留值并
  明确提示降级，不伪造公式。
- 公式栏和单元格编辑改为本地权威草稿，Enter、Tab、blur 和中文输入法完成后一次提交；提交时直接
  读取输入框当前值，修复快速输入后立即 Enter 偶发读到旧草稿、退出编辑态却未落值的问题。
- 公式编辑支持输入 `=` 后点击同 Sheet 单元格插入引用，也支持切换 Sheet 后继续点选；中文、空格和
  连字符 Sheet 名自动使用单引号引用。公式栏按引用着色，当前 Sheet 对应单元格显示同色虚线框。
- 复杂跨 Sheet 公式
  `='飞猪-海韵'!Q18+'小德果园-海韵'!R18+'酷狗音乐-学成'!R18`
  可计算、可点选录入，并继续兼容 Sheet 重命名迁移、缺失引用和公式原文保存。
- 格式刷右侧新增百分比、千分位分隔符、增加小数位数和减少小数位数；格式仅改变展示，不覆盖原值或
  公式，并进入工作簿撤销、重做和自动保存链路。
- 删除 Univer 四个依赖、传递依赖、编辑器组件、样式、适配器、测试、预览查询参数、localStorage
  开关和动态加载入口；Relation 原生表格成为唯一正式及长期实现路线。
- 已同步更新 `AGENTS.md`、`doc/文档中心-在线表格文档PRD.md`、
  `doc/文档中心-在线表格原生基础版PRD.md` 和 `doc/文档中心模块PRD.md`。

已验证：

- 前端全量测试通过：43 个测试套件、275 条测试全部通过；新增快速 Enter 直接提交输入框当前值、
  同/跨 Sheet 点选公式、复杂三 Sheet 公式、石墨剪贴板公式和数字格式撤销测试。
- 隔离生产构建通过，输出到 `/tmp/relation-native-spreadsheet-final`；前端性能门禁通过：首屏 JavaScript
  gzip `329.5KB / 400KB`，最大异步 chunk `420.8KB / 500KB`。构建产物扫描无 Univer 标识。
- 通过一次性内存 API 完成真实浏览器回归：同 Sheet `=F6+F7` 显示 5；上述三 Sheet 公式显示 60；
  公式 token、虚线引用框、外部公式粘贴、四个数字格式、撤销重做和快速中文输入均通过。
- `1280x800` 的 13 寸近似桌面视口和 `390x844` 移动视口均无页面横向溢出，网格和 44px Sheet 栏
  保持在视口内；浏览器无新增运行时异常，仅有项目既有 Ant Design 弃用警告。

验证边界：

- 本机没有隔离 MySQL 测试库凭据，浏览器回归使用一次性内存 API，不连接或修改真实业务库；自动保存、
  历史和协作接口沿用此前已通过的 operation API 与 READY-TEST 回归，本轮未改服务端协议。
- 浏览器剪贴板无法从不提供任何公式元数据的来源恢复公式语义；此类来源按 PRD 明确降级为结果值并提示。

## Relation 根目录文档整理（2026-07-30）

### 已完成

- 根目录仅保留 `AGENTS.md` 和 `handoff.md` 两个开发入口；其余 PRD、设计、SOP、报告和原型文档统一移入 `doc/`。
- 新增 `doc/README.md`，按使用与运维、核心 PRD、文档中心、AI/Agent、数据架构、原型和历史记录分类索引。
- 更新 `AGENTS.md`、根目录 `handoff.md` 和 `YYZ/handoff.md` 中的文档路径，并将文档间的本机绝对链接改为可移植相对链接。

### 已验证

- `doc/README.md` 覆盖当前 `doc/` 下除索引自身外的全部 49 份文档，本地 Markdown 链接全部可解析。
- 根目录已无其他 `.md` / `.html` / `.docx` 文档。
- 两份经营周会 `.docx` 已由用户移走，不在仓库文档目录中保留或索引。
- 静态扫描已覆盖前后端、服务端、迁移脚本、Wolai 导入器、AI Skill、部署脚本和 Dockerfile；没有运行时逻辑读取旧根目录文档路径。
- Git 原跟踪的 44 份根目录文档全部存在对应 `doc/` 目标文件；`git diff --check` 通过。
- `node --check server/index.js` 通过；前端隔离生产构建成功，产物写入 `/tmp/relation-doc-reorg-build`。
- 前端性能门禁通过：首屏 JavaScript gzip `328.0KB / 400KB`，最大异步 chunk `420.8KB / 500KB`。
