...
- 在线表格紧凑标题栏不再省略归属域，文件夹位置统一展示从归属域到当前文件夹的完整层级，例如
  `国内项目 / 产运 / 团队`。
- 文件夹树尚未加载或当前文件夹暂未匹配时，使用文档归属域、部门和文件夹名称生成兜底路径；正常宽度
  直接展示，空间不足时保持单行省略并通过 Tooltip 展示完整内容。

### 验证

- 最终合并树前端全量测试通过：45 个测试套件、291 条测试全部成功；仅有既有 React / Ant Design 警告。
- 隔离生产构建 `/tmp/relation-document-full-path-merged-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.8KB / 400KB`，76 个异步 chunk，最大 `420.8KB / 500KB`。
- `git diff --check` 通过；隔离前端可正常编译并进入登录页，未提交账号密码或改动业务数据。
...
- 产品模版页面增加“模版 / 代理”两个 Tab，代理支持 HTTP/HTTPS 配置的查看、新增、编辑、删除、启停和密码重新配置。
- 新增 `product_release_proxies` 表及代理 CRUD API；账号密码使用字段加密，接口、页面和日志均不回显密码。
- 提版创建前按主体 API、埋点、CDN 及短剧域名匹配启用代理，使用最长域名后缀、优先级、更新时间和 ID 选择代理；缺少目标匹配或 `*` 默认代理时阻断提版。
- 提版任务保存加密代理快照和非敏感匹配摘要，执行期间启动仅监听 `127.0.0.1` 的任务级 HTTP/HTTPS 转发器，并仅向 `upload.js` 子进程注入代理环境；成功、失败、取消和重试结束后关闭转发器。
- 产品资产提版进度和历史展示域名与代理名称摘要，不展示代理连接凭据。

### 验证

- `node --check server/index.js`、`node --check server/lib/productReleaseProxy.js` 通过。
- 代理工具单元测试、HTTP 转发 / HTTPS CONNECT 测试和提版域名集成测试通过。
- `ProductAssets.test.js`、`ProductTemplates.test.js` 通过，覆盖代理摘要、缺代理错误、Tab、筛选、CRUD、清除密码和只读态；前端全量测试 42 个套件、237 条测试全部通过。
- 后端全量测试使用 `/tmp` 隔离 AES/HMAC 密钥通过：141 条中 137 条通过、4 条按环境开关跳过；新增代理单元测试 10/10，提版域名集成测试 1/1。
- 隔离 MySQL `relation_test` 启动成功，新增代理表和提版快照字段迁移核对通过；服务启动在 `3101` 端口完成。
- 隔离生产构建 `/tmp/relation-proxy-final-build` 成功；性能门禁通过：首屏 JavaScript `330.9KB / 400KB`，
  77 个异步 chunk，最大 `422.4KB / 500KB`。
- 最终复核时用 `DB_CLIENT=mysql MYSQL_DATABASE=relation_test` 运行的本地后端仍监听 `3101`，
  `/api/auth/me` 未登录返回 `401`；MySQL 中确认 `product_release_proxies`、提版任务和历史表存在，账号、密码、域名后缀和备注字段为 `LONGTEXT`。

### 遗留

- 尚未执行真实代理供应商和真实支付宝上传；当前集成测试使用隔离数据库、假的 upload 脚本和本地代理服务。
...
- 在 `AGENTS.md` 增加长期约束：后续新增或改造同类业务页必须优先遵守该 UI 规范，不再新增零散
  页面级视觉风格。
- 新增可复用 `client/src/styles/businessPage.css`，沉淀统一页面治理栏、白底轻量指标卡、筛选工具条、
  表格密度、状态标签、移动端卡片等基础样式。
- 商机页 `/leads` 已接入新规范：顶部治理栏、统计概览、筛选区、表格和移动端卡片完成第一版视觉
  收敛；未改变接口、筛选语义、列顺序或新增/编辑/详情业务流程。
...
## 在线表格筛选多列选择与取消优化（2026-07-30）

### 已完成

- 列头支持按住 macOS `Command` / Windows `Ctrl` 增删任意非连续列，名称框和可见网格准确展示实际
  列集合；启用筛选后只在所选列标题展示漏斗，中间未选择列不被误纳入。
- 点击行标时以该行作为标题并自动延伸到最后已用数据行及全部已用列；点击第 1 行行标不再被误判为
  只有标题、没有数据。普通单元格、行标和切换 Sheet 会退出非连续列选择状态。
- 工具栏漏斗升级为主按钮和下拉菜单：已有筛选时主按钮直接取消，下拉提供更新筛选范围、清除筛选
  结果和取消筛选；清除结果保留范围与漏斗，取消筛选同时移除范围、条件和漏斗。
- `filterRange.columns` 保存非连续实际列索引，继续兼容旧连续范围；筛选条件、候选值和可见行只读取
  实际列集合，插入或删除列时范围、列集合和条件同步迁移，operation API 可原样保存。

### 验证

- 在线表格组件、工作簿与 operation 目标测试共 81 条通过；前端全量 44 个套件、286 条全部通过。
  服务端 operation 与 Excel 工作簿兼容测试 21 条通过，仅有既有 React / Ant Design 测试警告。
- 基于 Gitee `main=0b78ad9` 重放后，隔离生产构建
  `/tmp/relation-spreadsheet-filter-selection-rebased-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `328.0KB / 400KB`，75 个异步 chunk，最大 `420.8KB / 500KB`。
- 实际浏览器回归确认桌面端 B/D/F 非连续列的列头、单元格和漏斗一致；取消后漏斗全部移除，第 1 行
  行标可为 A-F 六个已用列启用筛选。`390x844` 下页面横向溢出为 0，筛选菜单贴合视口且无控制台错误。
...
- `node --check server/index.js`、`node --test server/lib/businessContactSearch.test.js` 通过；服务端全量中
  125 条非监听测试通过、4 条隔离 MySQL 测试按环境开关跳过，受沙箱回环监听限制的 11 条集成测试
  改在允许监听的隔离环境重跑并全部通过。
- `Interactions.test.js` 8/8 通过，新增覆盖五个叙述字段同时使用富文本编辑器；前端全量 44 个套件、
  282 条测试全部通过，在线表格合并后专项 38/38 通过。
- 隔离生产构建 `/tmp/relation-richtext-bottom-bar-final-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.8KB / 400KB`，76 个异步 chunk，最大 `420.8KB / 500KB`。
...
## 在线表格石墨式普通筛选（2026-07-30）

### 已完成

- 工具栏筛选从“当前值立即过滤”改为先对选区启用：选择单列只启用该列，选择连续多列、`Ctrl/Cmd+A`
  或左上角全选已用区域时，一次为范围内全部列启用；漏斗入口展示在真实标题单元格，不再占用列字母栏。
- 列筛选面板支持升序、降序、值搜索、全选、反选、多值勾选、值计数、应用和清除此列；多列条件按
  AND 组合，空白值可独立筛选。点击面板外或按 `Esc` 可关闭，最多渲染 500 个候选项以保护大表性能。
- `filterRange` 与 `filters` 分别保存筛选区域和条件，兼容旧 `equals/contains/not_empty` 条件；筛选
  进入自动保存、协作 operation、历史与撤销重做，并随行列增删迁移。只读、锁定区域及合并标题行
  不允许修改筛选；按颜色和复杂条件筛选继续保留给 Univer。

### 验证

- 工作簿工具测试 31 条、在线表格组件测试 37 条通过，目标 3 个套件共 76 条；覆盖单列启用、整表
  启用、标题单元格入口、多值勾选、空白与计数、多列 AND、撤销恢复、旧条件兼容和行列迁移。
- 前端全量 44 个套件、279 条测试全部通过；服务端 operation 与 Excel 工作簿兼容测试 21 条通过，
  仅有既有 React / Ant Design 测试警告。
- 隔离生产构建 `/tmp/relation-spreadsheet-filter-build-final` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.7KB / 400KB`，76 个异步 chunk，最大 `420.8KB / 500KB`。
- 使用生产构建实际筛选 CSS 在 `1440x800`、`390x844` 完成视觉回归：10 列标题入口均未越出单元格，
  桌面面板完整贴近目标列；窄屏面板宽 `316px` 并完整位于视口内，底部操作可达，页面横向溢出为 0。

## 工作台移动端任务卡片优化（2026-07-29）

### 已完成

- 移动端工作台任务改为与互动记录一致的紧凑信息卡：标题和描述优先，来源、状态、优先级、任务类型、
  日期、指派/执行/共享成员及结果分层展示；整卡点击或键盘操作可打开任务详情。
- 编辑和删除从卡片底部大按钮收进右上角 `···` 底部操作面板，详情抽屉沿用同一入口；删除增加二次
  确认，开始和完成继续作为高频快捷操作保留在卡片内。
- 权限保持原有边界：团队任务不新增编辑入口，“需关注”共享任务仍可编辑，删除仍仅限“待执行”中的
  普通任务；桌面端任务表格及操作列保持不变。

### 验证

- 工作台目标测试覆盖移动卡片信息层级、整卡查看详情、`···` 不误开详情、编辑入口、删除二次确认及
  桌面表格操作回归，4 条全部通过；前端全量 44 个套件、275 条测试全部通过，仅有既有 Ant Design /
  React 测试警告。
- 隔离生产构建 `/tmp/relation-dashboard-mobile-card-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.7KB / 400KB`，76 个异步 chunk，最大 `420.8KB / 500KB`。
- 使用生产构建中的实际卡片 CSS 在 `390x844`、`430x932` 完成移动视觉回归；长标题、描述、标签、
  日期、成员和结果均无重叠，卡片与底部操作面板没有横向溢出，操作面板始终完整贴底显示。

## 在线表格粘贴撤销与跨视口拖选（2026-07-29）

### 已完成

- 修复在线表格粘贴后焦点暂时落在文档外壳时，普通文档全局 `Ctrl/Cmd+Z` 抢占快捷键并提示“没有可
  撤回的操作”的问题；在线表格文档不再进入 blocks 撤销栈，粘贴事务由工作簿撤销栈一次恢复。
- 多格拖选新增四边热区连续自动滚动；滚动期间按鼠标位置和虚拟行列偏移持续计算选区，不再依赖当前
  DOM 中已渲染单元格的 `mouseenter`，可从当前视口一直选到末尾数据或工作表边界。

### 验证

- 在线表格编辑器与文档类型目标测试 2 个套件、38 条全部通过，覆盖石墨整列粘贴后 `Ctrl+Z` 将 B 列
  恢复为空、普通文档全局撤销不处理在线表格，以及从 B1 向下拖选时连续自动滚过虚拟视口、选到第
  22 行并在鼠标松开后停止。前端全量 43 个套件、271 条测试全部通过。
- 隔离生产构建 `/tmp/relation-spreadsheet-undo-drag-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.7KB / 400KB`，77 个异步 chunk，最大 `420.8KB / 500KB`。

## 在线表格外部公式粘贴降级（2026-07-29）

### 已完成

- 修复从石墨复制整列后，外部公式因 Relation 不支持的函数、命名范围或源 Sheet 引用而整列显示
  `#NAME?` 的问题。HTML 公式先在目标工作簿预计算；报错或结果与源显示值不一致时自动粘贴源显示值，
  并保留 HTML 基础样式；可正常计算且结果一致的公式继续保留。
- Relation 内部结构化剪贴板仍优先并完整保留公式、样式、合并、规则和行高列宽，不受外部降级影响。

### 验证

- 在线表格编辑器和剪贴板目标测试 2 个套件、39 条全部通过，覆盖石墨整列中的不可执行公式、结果不一致
  公式自动降级，以及可执行且结果一致的外部公式继续保留；组件粘贴回归确认 B 列写入标题和 `5575 / 6445`
  等源显示值而非 `#NAME?`。前端全量 43 个套件、269 条测试全部通过。
- 隔离生产构建 `/tmp/relation-spreadsheet-shimo-paste-build` 成功；性能门禁通过，首屏 JavaScript gzip
  `329.7KB / 400KB`，77 个异步 chunk，最大 `420.8KB / 500KB`。

## 在线表格选区统计数字格式（2026-07-29）

### 已完成

- Sheet 底栏的总和、平均、最大和最小统一四舍五入到最多两位小数；整数直接显示整数，不补 `.00`。

### 验证

- 在线表格组件 33 条测试通过，覆盖整数总和 `61` 和循环小数平均值 `20.33`，确认展示格式不修改
  工作簿或触发保存；前端全量 43 个套件、266 条测试全部通过。
...
## 在线表格滚动稳定与整表复制粘贴（2026-07-29）

### 已完成

- 列标题、行号、左上角交叉格和冻结单元格改为浏览器原生 sticky 固定轴；不再依赖 `scroll` 事件后
  用 React state 或 JavaScript transform 追赶滚动值，横向、纵向、斜向连续滚动时固定区域即时稳定。
- 网格焦点下 `Ctrl/Cmd+A` 选择当前 Sheet 有效数据区域；现有 Relation MIME、HTML、TSV、剪切、
  右键粘贴、条件格式、数据验证和保护校验链路保持兼容。
- Relation 结构化粘贴在已有值、公式、基础样式、合并及可迁移规则基础上，新增整张有效区域对应的
  行高列宽复制；相对公式按目标偏移，绝对引用保持，目标区域不足时自动扩展，整次粘贴一个撤销步骤。
- 单格、多格复制粘贴继续复用同一路径；公式栏、单元格编辑框及其他输入控件中的全选、复制和粘贴
  保持原生文字行为，不被表格区域快捷键截获。

### 验证

- 基于 `864f761` 的最新表格版本完成兼容集成；在线表格、剪贴板、工作簿和 operation 目标测试
  73 条全部通过，覆盖整表跨 Sheet、值/公式/样式/合并/规则/行高列宽、一次撤销、sticky 固定轴及
  输入框边界。
- 前端全量测试通过，43 个套件、266 条测试全部成功；仅有既有 Ant Design / React 测试警告。
- 隔离生产构建 `/tmp/relation-spreadsheet-copy-scroll-final-build` 成功；性能门禁通过，首屏 JavaScript
  gzip `329.7KB / 400KB`，77 个异步 chunk，最大 `420.8KB / 500KB`。
- 本地浏览器实际剪贴板回归中 Relation MIME、HTML、TSV 同时写入；整表粘贴保留公式、基础样式、
  行高列宽，单格、多格粘贴和单次 `Cmd+Z` 正常。`1280x720` 连续斜向滚动及 `390x844` 窄屏滚动时
  A1 与交叉格即时坐标漂移均为 `0px`，窄屏页面横向溢出为 `0px`。
