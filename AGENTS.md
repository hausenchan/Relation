...
本文件是仓库级长期上下文。开始任何开发任务前，先读本文件。Relation 组织中台功能开发再读
根目录 `handoff.md`；`YYZ/` 业务增长、数据分析、策略和 Skill 调优任务改读
`YYZ/AGENTS.md` 与 `YYZ/handoff.md`。长期稳定的产品约束写在对应 `AGENTS.md`，临时进度、
未完成项和验证结果只写在对应作用域的 `handoff.md`。

## 1. 信息读取顺序

1. `AGENTS.md`：架构、功能地图、耦合点、开发与提交约束。
2. 按任务作用域读取交接文件：
   - Relation 组织中台功能开发：根目录 `handoff.md`。
   - YYZ 业务增长与调优：`YYZ/AGENTS.md`、`YYZ/handoff.md`，不把进度写入根目录 handoff。
   - 同时修改 YYZ 与组织中台的跨域任务：分别写各自 handoff，只记录本侧进度并互相引用。
3. 与需求直接相关的 PRD：
...
- 工作台、媒体管理、人脉管理、互动记录、商机、公司研究、策略、需求、产品资产、主体管理、产品模版
  及后续同类业务列表页必须遵守 `Relation业务中台UI规范.md`。
- 业务中台页面以安静、紧凑、专业和高频操作效率为目标；不得新增营销页式 hero、装饰性背景、大面积
  高饱和渐变统计卡或与列表工作流无关的说明文案。
- 新增或改造业务列表页时，优先沉淀统一页面外壳、筛选条、指标卡、表格、移动卡片和操作按钮样式；
  不得继续以页面级零散 inline `fontSize`、`borderRadius`、`boxShadow` 作为主要视觉方案。
- 表格、筛选、Tag、按钮、抽屉和移动卡片的字号、间距、圆角、颜色和状态语义必须跨页面一致；现有
  页面局部样式与 UI 规范冲突时，后续改造应以 UI 规范为准。
...
- 工作台移动端任务列表采用与互动记录一致的紧凑信息卡层级，整卡点击进入详情；编辑和删除收进
  右上角 `···` 底部操作面板，删除必须二次确认，开始和完成保留为卡片内高频快捷操作。团队任务
  不得因此获得编辑入口，“需关注”共享任务继续按既有权限可编辑；桌面表格操作列保持不变。
...
- 桌面非嵌入在线表格使用全高紧凑工作区：文档标题、自动保存状态、文件夹路径、页面、分享、历史和
  收藏收在单行治理栏，不再重复展示大标题、标签组或创建人元信息。文件夹位置必须从归属域开始展示
  至当前文件夹的完整层级，例如 `国内项目 / 产运 / 团队`；空间不足时可单行省略，但 Tooltip 必须保留
  完整路径。网格是唯一纵向伸缩/滚动区域，
  底部 Sheet 与缩放栏必须始终留在当前视口内。该紧凑模式不得改变普通文档、嵌入式文档和移动端布局。
- 在线表格底部 Sheet 工具带固定为 `44px` 浅灰背景，活动 Sheet 使用 `38px` 白底灰色描边标签，
  非活动 Sheet 使用平面文字和竖向分隔线，不得恢复蓝色下划线或 Ant Design 主按钮态。控件顺序固定为
  “Sheet 列表 / 新增 / Sheet 标签 / 选区统计 / 视图 / 全屏 / 缩小 / 百分比 / 放大”；窄屏优先隐藏
  选区统计、视图和全屏，Sheet 标签改为内部横向滚动，底栏本身不得换行、增高或造成页面横向溢出。
- 桌面非嵌入在线表格的网格纵向滚动后自动收起文档标签与标题治理栏，回到顶部自动展开；工具栏右侧
  固定提供专注模式箭头，开启后额外收起表格菜单栏，使工具栏直接承接全局页面标签。工具栏、公式栏、
  网格和 Sheet 栏不得被收起；箭头不得随工具栏横向滚动移出视口。短内容和单纯横向滚动不得触发自动
  收起，移动端、嵌入式文档和普通文档继续保持原布局。
- 当前原生在线表格格式刷仅复制已有单元格 `style`，不得复制值、公式、合并关系、行高或列宽；单击
  使用一次，双击连续使用，再次点击或按 `Esc` 退出。来源多格按二维图案平铺到目标选区，来源缺失的
  样式必须清除目标对应旧样式；每次目标应用作为一个撤销步骤，只读用户不可启用。该基础能力不改变
  完整条件格式、数字格式和高级样式继续优先由 Univer 承载的长期边界。
- 当前原生在线表格普通筛选支持单列、连续多列、按住 macOS `Command` / Windows `Ctrl` 点选的任意
  非连续多列，以及以所选行作为标题的全部已用列；点击第 1 行行标时须自动延伸到最后已用数据行。
  `filterRange` 保存标题行和筛选边界，非连续列通过其 `columns` 保存实际列索引；标题单元格只在实际
  筛选列展示入口。每列支持升降序、值搜索、全选/反选、多值勾选和清除此列，多列条件按 AND 组合。
  “清除筛选结果”只清空条件并保留漏斗，“取消筛选”同时移除范围、条件和漏斗。筛选范围和条件必须
  进入自动保存、协作差异与撤销重做，并随行列增删迁移；只读或锁定用户不得修改。按颜色、复杂条件
  和高级组合筛选仍由 Univer 承载，不继续扩展兼容引擎。
- 当前原生在线表格已提供核心右键菜单和版本化基础数据治理规则，作为 Univer 接入前的兼容能力；
  后续扩展数据条、色阶、图标集、复杂验证控件或完整 Excel 格式时仍应优先由 Univer 承载，不能继续
  把当前兼容实现扩展成另一套完整表格内核。
...
- 人脉 `/persons`：圈子、关系类型、人才字段、标签、附件上传/预览/下载、指派和导入。桌面列表的
  前五列固定为“姓名 / 公司 / 创建人 / 城市 / 圈子”，公司优先展示 `company`，为空时回退
  `current_company`；统一搜索继续覆盖姓名、公司、手机、标签、技能和关键成事特质。
- 互动 `/interactions`：拜访/通话/微信等记录，可衍生提醒和待跟进任务。桌面列表的核心顺序为
  “姓名 / 公司 / 创建人 / 圈子”，公司实时取关联人脉的当前公司字段，不复制到互动表；创建人通过
  用户关联一次返回。统一关键字搜索在服务端解密后覆盖姓名、公司、描述、结果、跟进事项、礼物和
  商机标题，确保筛选后的列表总数准确。移动端卡片必须显示创建人；筛选控件收入口径与人脉管理一致，
  使用“筛选”按钮打开全屏抽屉，“完成”统一应用关键词和城市输入，“重置”清空全部筛选条件。
  移动卡片按姓名、可选公司、日期、圈子/互动类型/信息重要程度、城市/创建人/中文人脉权重及正文摘要
  分层展示；公司为空不显示占位符，`high/medium/low` 不得直接暴露。整卡可点击并支持键盘打开详情，
  列表卡禁止文字误选；编辑、删除收入口径统一的 `···` 底部操作面板，删除必须二次确认，详情也要提供
  同一操作入口。桌面商机固定拆成“商机标题 / 商机状态 / 商机类型”三列，标题单行省略并悬浮显示
  全文，状态和类型标签不得换行或撑高表格行。
- 互动来源商机的“商机说明 / 跟进结果 / 描述 / 结果 / 下一步行动”统一复用互动描述的富文本编辑器；
  商机统一弹窗、互动记录页和人脉/客户/人才详情内的复用表单必须保持一致。存量纯文本无需迁移，
  编辑器继续兼容；列表摘要统一去除 HTML，详情统一走安全富文本渲染。下一步行动生成提醒时，服务端
  必须转换为纯文本标题和备注，语义为空的富文本不得生成空提醒。
...
- 出差协同手机端不得直接压缩或横向铺开桌面双向网格；小于 `md` 断点时默认使用单日纵向议程，
  同时提供“全部”模式按日期和时段纵向分组。日期条可原生横向惯性滚动并提供前后日期按钮，正文
  只保留原生纵向滚动；从时段标题、空白区或日程列表起滑均不得被嵌套网格阻断。默认选中行程范围
  内的今天，今天不在范围内时选中第一天，桌面冻结网格行为保持不变。
- 预算 `/budgets`。
...
- 所有新增功能、字段语义、通知、权限、列表操作和展示口径都必须显式检查存量数据是否已有旧字段、
  空字段、旧枚举、缺关联记录或历史默认值；不能只覆盖新建和编辑保存链路。
- 需要新派生记录、反向索引、通知接收人、描述快照、权限关系或状态字段时，必须提供幂等启动迁移或
  可重复执行的补偿逻辑，并避免给历史数据补发旧事件通知。补偿逻辑要同时覆盖互动、公司研究、
  商机、普通任务等相关来源，不能只覆盖其中一个入口。
- 涉及旧数据兼容的修复必须补自动化测试：至少包含“已有旧记录未重新保存”的场景，验证重启/迁移后
  列表展示、操作按钮、权限、通知和状态流转仍按新口径生效。
- PRD 或 handoff 记录新口径时，要写清楚存量数据处理方式，包括是否回填、是否跳过历史通知、
  是否保留已完成记录的历史负责人和结果。
...
- 在线表格维护独立的工作簿撤销/重做栈；公式栏和单元格内连续输入按一次提交记录，排序、合并、
  ...
  Sheet、移动选区或缩放视图不占用撤销步骤。文档中心外壳的普通文档全局撤销监听不得拦截在线表格
  的 `Ctrl/Cmd+Z`；工作簿历史必须同时限制条数和总内存，不能为大型导入表格长期保留无限完整对象快照。
- 在线表格字体、字号、粗体、斜体、文字色、填充色、边框、对齐、换行和数字格式也属于工作簿事务，
  必须在修改前记录撤销快照；升降序都要稳定保留标题并把空白放在有效数据之后。公式格在网格显示
  计算值、在公式栏保留原文，同 Sheet 引用用彩色虚线标识；公式解析不得把字符串中的运算符或伪坐标
  当作表达式或引用。
- 选择单列或列头排序时，该列只作为排序键，实际移动范围必须横向扩展到当前 Sheet 已用数据区的全部
  列，整行原子移动并进入同一个撤销步骤；锁定权限也要按扩展后的真实影响范围检查，不能只校验选中列。
- 在线表格右键菜单使用当前选区并在视口边缘自动翻转，至少支持剪切/复制/粘贴、插入复制单元格、
  清除、插入/删除单元格及行列、排序、合并、数字格式、锁定、条件格式和数据验证。结构化剪贴板同时
  输出 Relation MIME、HTML 和 TSV，粘贴时按该顺序降级；值、公式、基础样式、合并和可迁移规则可
  复制，锁定规则不得随复制扩散，公式引用按目标位置相对迁移。外部 HTML 携带公式元数据时须先在
  目标工作簿预计算；公式报错或结果与源显示值不一致时必须粘贴源显示值，不得用 `#NAME?/#REF!`
  覆盖剪贴板中的有效数据。Relation 内部结构化复制仍完整保留公式。
- 每个 Sheet 的 `protectedRanges`、`conditionalFormats` 和 `dataValidations` 都是工作簿持久化字段，
  规则必须有稳定且唯一的 ID、合法范围和受限大小，并随行列插入删除迁移。条件格式只叠加派生显示
  样式，不覆盖原始值、公式或手工样式；数据验证的直接输入、公式栏、粘贴、清空和 API 写入必须使用
  一致的拒绝/警告口径，列表验证在单元格内提供下拉选择。
- 锁定范围仍允许查看、选择和复制；文档管理者可创建、调整或解除锁定，规则创建人和明确授权成员可
  编辑范围内容。operation API 与整本内容保存都必须在落库前校验锁定和数据验证，拒绝时分别返回
  `SPREADSHEET_PROTECTED_RANGE`、`SPREADSHEET_PROTECTION_MANAGE_REQUIRED` 或
  `SPREADSHEET_DATA_VALIDATION_FAILED`，整个批次不得部分落库、更新时间或生成历史记录。
- 在线表格冻结入口使用石墨式下拉菜单，动态提供当前行、当前列、当前行和列及取消冻结四种操作；
  单独冻结行或列时只更新对应维度并保留另一维，支持先后操作组合冻结，未冻结时取消项禁用。冻结
  状态按 Sheet 持久化并进入撤销、自动
  保存、operation 和页面历史，继续限制为最多前 100 行、50 列；只读用户不得打开冻结菜单。
- 点击在线表格行号必须选择该 Sheet 整行，点击列字母必须选择整列；表头和选区使用连续浅蓝背景及
  蓝色边界，选择本身不产生 dirty 或保存记录。整行整列选区继续复用复制、清空、格式和 presence，
  调整行高列宽的拖动手柄不得误触发整行整列选择。
- 普通连续多格选区使用一个完整蓝色外框，活动格保留原始背景且不得再显示独立粗框，其余选中格使用
  浅蓝底色，右下角显示填充柄。两个及以上单元格且存在内容时，Sheet 底栏提供总和、平均、最大、
  最小、计数和数值计数；统计数字最多保留两位小数，整数不得补 `.00`。公式按计算值参与，统计须
  遍历稀疏单元格且不得修改工作簿、触发保存或历史。鼠标拖选进入网格四边热区时必须连续自动滚动，
  并按滚动后的虚拟行列坐标持续扩展选区，不能受当前 DOM 只渲染可视行列的限制。
- 在线表格网格获得焦点时，`Ctrl/Cmd+A` 选择当前 Sheet 有效数据区域；公式栏、单元格编辑框和其他
  输入控件中的全选继续只作用于文字。结构化粘贴除值、公式、基础样式、合并和可迁移规则外，整张
  有效区域复制还须保留对应行高列宽，并作为一个撤销步骤；目标区域不足时自动扩展行列。
- 在线表格列标题、行号、左上角交叉格和冻结单元格必须使用浏览器原生 sticky 固定轴，不得依赖
  `scroll` 事件后用 React state 或 JavaScript transform 追赶滚动位置；横向、纵向、斜向连续滚动及
  边界回弹时固定轴屏幕坐标不得发生单帧漂移，网格自身须阻断滚动链回弹。
...
- 桌面端全局侧栏必须固定铺满视口高度，不能依赖会受祖先 `overflow` 影响的 `sticky`；右侧内容区
  按展开/收起宽度同步偏移。折叠入口固定在侧栏底部，不占用顶部 Header；菜单内容独立滚动，长页面
  和短视口下底部入口均不得随正文滚动。收起后的图标轨道固定为 `52px`，展开态保持 `180px`；移动端
  继续使用 Header 菜单入口和抽屉导航。
- 文档目录、出差列表、AI 训练会话等模块侧栏统一复用 `SidebarToggleButton`，折叠入口放在模块主内容
  顶部工具区，不得留在待折叠侧栏内。桌面折叠后侧栏宽度、边框、内边距和父级间距均为零；窄屏采用
  上下布局或移动端原流程，不得因桌面端持久化折叠状态隐藏主列表。
...
2. 读当前任务作用域对应的 `handoff.md` 和目标模块 PRD，搜索现有实现、API、权限和测试；YYZ
   业务任务使用 `YYZ/handoff.md`，不读取或更新根 handoff，除非同时修改组织中台。
...
6. 涉及 schema 或启动迁移时，用隔离的 MySQL 测试库启动服务并记录迁移耗时；涉及 UI 时
   用浏览器走核心路径。
7. 完成后更新当前任务作用域对应的 `handoff.md`，列出验证结果和遗留风险；跨域任务分别更新。
