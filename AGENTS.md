...
- 经营周会提纲是长文本结构化生成任务，服务端模型等待时间不得低于 120 秒，前端等待窗口须更长；
  不得复用通用模型 25 秒默认值。输出固定六段并覆盖每份非空的已提交准备内容。
- 经营周会提纲生成接口必须在服务端完成生成、校验、持久化、历史和周会状态更新后才返回成功；
  前端不得再把生成与保存拆成两个请求。旧前端重复保存同一 `source_hash` 和提纲时必须幂等成功，
  不重复更新时间或历史。生成中的进度及失败原因必须在提纲区域持续可见，不能只依赖短暂 toast。
