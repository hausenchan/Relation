...
- 在线表格统一使用 Relation 原生内核作为正式入口和长期唯一实现路线；核心交互位于
  `SpreadsheetDocumentEditor.js`，工作簿、公式、排序和结构迁移位于 `spreadsheetWorkbook.js`，
  Excel 文件能力位于服务端 `spreadsheetWorkbookFile.js`。不得重新引入 Univer、双编辑器、预览开关、
  动态加载入口或引擎专属持久化格式；复杂能力继续在同一原生模型上分层扩展。
...
- 当前原生在线表格格式刷仅复制已有单元格 `style`，不得复制值、公式、合并关系、行高或列宽；单击
  ...
  样式必须清除目标对应旧样式；每次目标应用作为一个撤销步骤，只读用户不可启用。完整条件格式、
  数字格式和高级样式继续沿原生版本化模型扩展。
- 当前原生在线表格已提供核心右键菜单和版本化基础数据治理规则；后续扩展数据条、色阶、图标集、
  复杂验证控件或完整 Excel 格式时必须复用同一工作簿、权限、撤销、保存、历史和 operation 链路，
  不得形成第二套表格内核或只存在于 DOM/React 状态的规则。
...
- 在线表格只允许原生编辑器读写 `relation_spreadsheet_workbook_v1` 及其后续引擎无关版本；
  `Documents.js` 只负责文档外壳、权限、保存、历史、协作和附件上下文，不能承载公式解析、网格状态
  或第三方引擎适配。Univer 依赖、组件、样式、适配器、预览参数和动态加载入口必须保持删除状态。
- 在线表格公式原文必须保留，不得用计算值覆盖公式原文；公式必须经过词法解析、语法解析、依赖图和
  安全求值器，禁止使用 `eval` 或 `Function`。Excel 导入导出走专用文件适配层，服务端仍需执行权限、
  大小、Sheet 名和敏感内容校验。
- 单元格内编辑和公式栏编辑先维护本地权威草稿，`Enter`/`Tab` 或失焦时一次性提交；中文输入法组合
  输入、虚拟滚动、React 重绘、自动保存和远端轮询不得造成偶发无法输入、丢字或未变更却显示保存成功。
  拒绝原因来自只读、锁定或数据验证时必须明确提示。
- 输入 `=` 后点击单元格或拖选区域必须在当前光标处插入公式引用而不是切走目标公式；跨 Sheet 点选按
  Excel 语法自动引用并对中文、空格、连字符和单引号名称正确转义。公式栏保留原文并以稳定颜色标记
  引用，当前可见 Sheet 使用同色虚线框，跨 Sheet 不得错框当前 Sheet 的同坐标。
- 必须支持多个带引号 Sheet 连续参与运算，例如
  `='飞猪-海韵'!Q18+'小德果园-海韵'!R18+'酷狗音乐-学成'!R18`；三个引用均进入依赖图，任一上游
  修改立即重算。保存刷新、历史恢复、协作合并、Sheet 重命名和 Excel 往返必须保留公式；缺 Sheet
  显示 `#REF!` 但不得用缓存值覆盖公式原文。
- 从石墨、Excel、WPS 或飞书整表/区域粘贴时，按 Relation 结构化 MIME、来源结构化 MIME、HTML
  公式元数据、HTML 表格和 TSV 顺序降级；只要剪贴板暴露公式就保留原文和引用。来源仅提供计算值时
  必须明确提示“源内容未提供公式，仅粘贴结果值”，不得反推公式或静默宣称保真。
- 格式刷右侧固定提供百分比、千分位分隔符、增加小数位和减少小数位；四项只改变显示格式，不修改
  原始值或公式，每次点击作为一个可撤销事务进入自动保存、页面历史、协作和 Excel 往返。
...
- 普通连续多格选区使用一个完整蓝色外框，活动格保留原始背景且不得再显示独立粗框，其余选中格使用
  ...
  最小、计数和数值计数；公式按计算值参与，统计须遍历稀疏单元格且不得修改工作簿、触发保存或历史。
...
- 在线表格附件、图片或文件对象必须通过 Relation 文档附件权限和存储层上传、预览、下载和删除；
  工作簿内只保存受控引用 ID/URL 和展示元数据，不得绕过附件权限或直接持久化本地临时路径。
...
| `SpreadsheetDocumentEditor.js` | 原生唯一编辑器；石墨像素级 UI、本地输入草稿、公式点选、选区、快捷键、右键菜单、数字格式、只读态、保存与历史桥接 |
| `spreadsheetWorkbook.js` | 原生工作簿、公式解析/依赖/安全计算、引用迁移、排序筛选、格式规则、旧数据兼容和 `content_text` 派生 |
| `spreadsheetClipboard.js` | Relation/来源结构化 MIME、HTML 公式元数据、TSV 降级、石墨公式保真、相对引用迁移和降级提示 |
| `spreadsheetWorkbookFile.js` | `.xlsx/.xlsm` 往返、公式与样式保真、服务端限制和临时文件清理 |
...
- 在线表格：普通文档兼容、原生工作簿读写、石墨像素级 UI 截图验收、本地草稿直接输入、公式点选、
  带中文/连字符/单引号的多 Sheet 公式、石墨公式粘贴与降级提示、四个数字格式快捷动作、
