...
- 当前原生在线表格普通筛选通过选中单列、连续多列或全部已用区域启用，`filterRange` 保存标题行和
  筛选列范围，标题单元格展示筛选入口；每列支持升降序、值搜索、全选/反选、多值勾选和清除此列，
  多列条件按 AND 组合。筛选范围和条件必须进入自动保存、协作差异与撤销重做，并随行列增删迁移；
  只读或锁定用户不得修改。按颜色、复杂条件和高级组合筛选仍由 Univer 承载，不继续扩展兼容引擎。
