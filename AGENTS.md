...
- 人脉 `/persons`：圈子、关系类型、人才字段、标签、附件上传/预览/下载、指派和导入。桌面列表的
  核心顺序为“姓名 / 公司 / 圈子”，公司优先展示 `company`，为空时回退 `current_company`；统一搜索
  继续覆盖姓名、公司、手机、标签、技能和关键成事特质。
- 互动 `/interactions`：拜访/通话/微信等记录，可衍生提醒和待跟进任务。桌面列表的核心顺序为
  “圈子 / 公司 / 姓名 / 日期”，公司实时取关联人脉的当前公司字段，不复制到互动表；统一关键字搜索
  在服务端解密后覆盖姓名、公司、描述、结果、跟进事项、礼物和商机标题，确保筛选后的列表总数准确。
